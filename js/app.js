...
function cacheDom() {
  ...
  dom.exportRecordExcelBtn = document.getElementById("exportRecordExcelBtn");
...
function bindEvents() {
  ...
  dom.exportRecordExcelBtn.addEventListener("click", handleExportRecordExcel);
  ...
  dom.form.cliente.addEventListener("keydown", handleGeneralFormEnterKeydown);
  ...
  dom.form.clienteCustom.addEventListener("keydown", handleGeneralFormEnterKeydown);
  dom.form.proto.addEventListener("input", handleProtoInput);
  dom.form.proto.addEventListener("keydown", handleGeneralFormEnterKeydown);
  dom.form.version.addEventListener("input", handleVersionInput);
  dom.form.version.addEventListener("keydown", handleGeneralFormEnterKeydown);
  dom.form.idem.addEventListener("input", handleIdemInput);
  dom.form.idem.addEventListener("keydown", handleGeneralFormEnterKeydown);
  [
  ...
  [
    ...
    dom.form.tela,
    dom.form.realizadoPor,
    dom.form.produccionEstimada,
    dom.form.rutasProcesos,
  ].forEach((input) => input.addEventListener("keydown", handleGeneralFormEnterKeydown));
...
function getGeneralFormFieldOrder() {
  return [
    dom.form.cliente,
...
function canFocusGeneralFormField(field) {
  ...
  if (field.classList?.contains("hidden") || field.type === "hidden") {
    return false;
  }
...
function focusNextGeneralFormField(currentField) {
  const orderedFields = getGeneralFormFieldOrder();
  ...
  for (let index = currentIndex + 1; index < orderedFields.length; index += 1) {
    const nextField = orderedFields[index];

    if (!canFocusGeneralFormField(nextField)) {
      continue;
    }
...
function handleGeneralFormEnterKeydown(event) {
  ...
  if (typeof currentField.blur === "function") {
    currentField.blur();
  }

  window.requestAnimationFrame(() => {
    focusNextGeneralFormField(currentField);
  });
}
...
function buildMasterXlsxWorksheetXml(model) {
  ...
  const lastRow = Math.max(...model.rows.map((row) => row.rowNumber), 1);
...
function getRecordExcelFileName(record) {
  const now = new Date();
  ...
  const proto = normalizeProtoValue(dom.form.proto?.value || record?.proto || "") || "00000";
  const version = normalizeShortIntegerValue(dom.form.version?.value || record?.version || "");
  const versionLabel = version ? `v${version}` : "sin_version";

  return `cotizacion_${proto}_${versionLabel}_${year}${month}${day}_${hours}${minutes}${seconds}.xlsx`;
}

function getRecordClientDisplayValue() {
  if (!dom.form?.cliente) {
    return "";
  }

  if (dom.form.cliente.value === NEW_CLIENT_OPTION_VALUE) {
    return String(dom.form.clienteCustom?.value || "").trim();
  }

  return getControlDisplayValue(dom.form.cliente);
}

function getRecordGeneralInfoEntries() {
  return [
    { label: "Cliente", value: getRecordClientDisplayValue() },
    { label: "PROTO", value: String(dom.form.proto?.value || "").trim() },
    { label: "Version", value: String(dom.form.version?.value || "").trim() },
    { label: "IDEM", value: String(dom.form.idem?.value || "").trim() },
    { label: "Estilo", value: String(dom.form.estilo?.value || "").trim() },
    { label: "Tela", value: String(dom.form.tela?.value || "").trim() },
    { label: "Descripcion", value: String(dom.form.descripcion?.value || "").trim() },
    { label: "Realizado por", value: String(dom.form.realizadoPor?.value || "").trim() },
    { label: "Produccion estimada", value: String(dom.form.produccionEstimada?.value || "").trim() },
    { label: "Rutas y procesos", value: String(dom.form.rutasProcesos?.value || "").trim() },
  ];
}

function getCosturaGeneralInfoGridRows() {
  return [
    [
      { label: "Cliente", value: getRecordClientDisplayValue() },
      { label: "Produccion estimada", value: String(dom.form.produccionEstimada?.value || "").trim() },
...
function buildRecordXlsxBorder(color = "FFD5DDE7") {
  return {
...
function createRecordXlsxStylePalette(registry) {
  const createStyle = (styleDefinition) => registerMasterXlsxCellStyle(registry, styleDefinition);

  return {
    title: createStyle({
      alignment: { horizontal: "center", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder("FF16335F"),
      fill: { color: "FF16335F", patternType: "solid" },
      font: { bold: true, color: "FFFFFFFF", fontName: "Calibri", fontSize: 14, italic: false },
    }),
    infoLabel: createStyle({
      alignment: { horizontal: "left", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder(),
      fill: { color: "FFF7E4BC", patternType: "solid" },
      font: { bold: true, color: "FF16335F", fontName: "Calibri", fontSize: 11, italic: false },
    }),
    infoValue: createStyle({
      alignment: { horizontal: "left", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder(),
      fill: { color: "FFFFFFFF", patternType: "solid" },
      font: { bold: false, color: "FF18212F", fontName: "Calibri", fontSize: 11, italic: false },
    }),
    tableHeader: createStyle({
      alignment: { horizontal: "center", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder("FF1874B7"),
      fill: { color: "FF1874B7", patternType: "solid" },
      font: { bold: true, color: "FFFFFFFF", fontName: "Calibri", fontSize: 11, italic: false },
    }),
    tableCell: createStyle({
      alignment: { horizontal: "left", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder(),
      fill: { color: "FFFFFFFF", patternType: "solid" },
      font: { bold: false, color: "FF18212F", fontName: "Calibri", fontSize: 11, italic: false },
    }),
    tableCellCenter: createStyle({
      alignment: { horizontal: "center", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder(),
      fill: { color: "FFFFFFFF", patternType: "solid" },
      font: { bold: false, color: "FF18212F", fontName: "Calibri", fontSize: 11, italic: false },
    }),
    footerLabel: createStyle({
      alignment: { horizontal: "right", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder("FF111111"),
      fill: { color: "FF111111", patternType: "solid" },
      font: { bold: true, color: "FFFFFFFF", fontName: "Calibri", fontSize: 11, italic: false },
    }),
    footerValue: createStyle({
      alignment: { horizontal: "center", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder("FF111111"),
      fill: { color: "FF111111", patternType: "solid" },
      font: { bold: true, color: "FFFFFFFF", fontName: "Calibri", fontSize: 11, italic: false },
    }),
    summaryLabel: createStyle({
      alignment: { horizontal: "left", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder(),
      fill: { color: "FFFFFFFF", patternType: "solid" },
      font: { bold: false, color: "FF18212F", fontName: "Calibri", fontSize: 11, italic: false },
    }),
    summaryValue: createStyle({
      alignment: { horizontal: "center", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder(),
      fill: { color: "FFFFFFFF", patternType: "solid" },
      font: { bold: true, color: "FF18212F", fontName: "Calibri", fontSize: 11, italic: false },
    }),
    summaryHighlightLabel: createStyle({
      alignment: { horizontal: "left", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder("FFF1D48D"),
      fill: { color: "FFF7E4BC", patternType: "solid" },
      font: { bold: true, color: "FF16335F", fontName: "Calibri", fontSize: 11, italic: false },
    }),
    summaryHighlightValue: createStyle({
      alignment: { horizontal: "center", vertical: "center", wrapText: true },
      border: buildRecordXlsxBorder("FFF1D48D"),
      fill: { color: "FFF7E4BC", patternType: "solid" },
      font: { bold: true, color: "FF16335F", fontName: "Calibri", fontSize: 11, italic: false },
...
function parseRecordSheetWidth(rawWidth, fallback = 14) {
  ...
  if (widthValue.endsWith("%")) {
    ...
    return Math.max(8, Math.min(Math.round(percent * 0.7 * 100) / 100, 24));
  }

  if (widthValue.endsWith("px")) {
    return Math.max(8, Math.min(pixelsToExcelWidth(parseFloat(widthValue)), 24));
  }

  return fallback;
}

function getRecordSheetColumnWidths(tableElement, minimumColumns = 2) {
  const columnWidths = Array.from(tableElement?.querySelectorAll("colgroup col") || []).map((colElement) =>
    parseRecordSheetWidth(colElement.style.width, 14)
  );

  while (columnWidths.length < minimumColumns) {
    columnWidths.push(columnWidths.length === 0 ? 18 : 22);
  }

  return columnWidths;
}

function sanitizeRecordSheetName(name, fallback = "Hoja") {
...
function isRecordSheetCellCentered(cellElement) {
  const control = cellElement.querySelector("input, select, textarea");
  ...
  return (
    control.classList.contains("center-cell") ||
    control.classList.contains("number-cell")
  );
}

function resolveRecordSheetCellStyle(cellElement, rowElement, tableElement, sectionTagName, colIndex, styles) {
  const normalizedSection = String(sectionTagName || "").toLowerCase();
  const isResumenTable = tableElement.classList.contains("resumen-table");
  const colSpan = Math.max(1, Number(cellElement.getAttribute("colspan") || 1));

  if (normalizedSection === "thead") {
    return styles.tableHeader;
  }

  if (normalizedSection === "tfoot") {
    if (colIndex === 0 && (colSpan > 1 || cellElement.classList.contains("tfoot-label"))) {
      return styles.footerLabel;
    }

    return styles.footerValue;
  }

  if (isResumenTable) {
    if (rowElement.classList.contains("resumen-highlight-row")) {
      return colIndex === 0 ? styles.summaryHighlightLabel : styles.summaryHighlightValue;
    }

    return colIndex === 0 ? styles.summaryLabel : styles.summaryValue;
  }

  return isRecordSheetCellCentered(cellElement) ? styles.tableCellCenter : styles.tableCell;
}

function buildRecordGeneralInfoRows(columnCount, styles) {
  const rows = [];
  const merges = [];
  const lastColIndex = Math.max(columnCount - 1, 1);
  let rowNumber = 1;

  rows.push({
    rowNumber,
    height: 28,
    cells: [{ colIndex: 0, styleId: styles.title, value: "INFORMACION GENERAL DE LA PRENDA" }],
  });

  if (lastColIndex > 0) {
    merges.push(`A${rowNumber}:${columnIndexToExcelName(lastColIndex)}${rowNumber}`);
  }

  rowNumber += 1;

  getRecordGeneralInfoEntries().forEach((entry) => {
    rows.push({
      rowNumber,
      height: entry.value.length > 36 ? 34 : 24,
      cells: [
        { colIndex: 0, styleId: styles.infoLabel, value: entry.label },
        { colIndex: 1, styleId: styles.infoValue, value: entry.value || "" },
      ],
    });

    if (lastColIndex > 1) {
      merges.push(`B${rowNumber}:${columnIndexToExcelName(lastColIndex)}${rowNumber}`);
    }

    rowNumber += 1;
  });

  rows.push({ rowNumber, height: 12, cells: [] });
  rowNumber += 1;
  rows.push({ rowNumber, height: 12, cells: [] });
...
function buildCosturaGeneralInfoRows(columnCount, styles) {
  const rows = [];
  const merges = [];
  const lastColIndex = Math.max(columnCount - 1, 1);
  let rowNumber = 1;
  ...
  getCosturaGeneralInfoGridRows().forEach((entryRow) => {
    const cells = [];

    entryRow.forEach((entry, pairIndex) => {
      const labelColIndex = pairIndex * 2;
      const valueColIndex = labelColIndex + 1;

      if (labelColIndex >= columnCount || valueColIndex >= columnCount) {
        return;
      }

      cells.push({ colIndex: labelColIndex, styleId: styles.infoLabel, value: entry.label });
      cells.push({ colIndex: valueColIndex, styleId: styles.infoValue, value: entry.value || "" });
    });
...
function buildRecordTableRows(tableElement, startRowNumber, styles) {
  const rows = [];
  const merges = [];
  let rowNumber = startRowNumber;

  Array.from(tableElement.querySelectorAll("thead tr, tbody tr, tfoot tr")).forEach((rowElement) => {
    const sectionTagName = rowElement.parentElement?.tagName || "tbody";
    const cells = [];
    let colIndex = 0;

    Array.from(rowElement.children)
      .filter((cellElement) => cellElement.tagName === "TH" || cellElement.tagName === "TD")
      .forEach((cellElement) => {
        const colSpan = Math.max(1, Number(cellElement.getAttribute("colspan") || 1));
        const cellValue = getMasterCellDisplayText(cellElement);
        const styleId = resolveRecordSheetCellStyle(
          cellElement,
          rowElement,
          tableElement,
          sectionTagName,
        ...
        if (colSpan > 1) {
          merges.push(
            `${columnIndexToExcelName(colIndex)}${rowNumber}:${columnIndexToExcelName(colIndex + colSpan - 1)}${rowNumber}`
          );
        }
    ...
    rows.push({
      rowNumber,
      height: sectionTagName === "THEAD" ? 28 : 24,
      cells,
    });
    rowNumber += 1;
  });
...
function buildRecordSheetModel(sheetName, tableElement, registry, styles) {
  if (!tableElement) {
    return null;
  }

  const columns = getRecordSheetColumnWidths(tableElement, 2);
  const generalInfo =
    String(sheetName || "").toUpperCase() === "COSTURA"
      ? buildCosturaGeneralInfoRows(columns.length, styles)
      : buildRecordGeneralInfoRows(columns.length, styles);
  const tableRows = buildRecordTableRows(tableElement, generalInfo.nextRowNumber, styles);

  return {
    columns,
    merges: [...generalInfo.merges, ...tableRows.merges],
    name: sanitizeRecordSheetName(sheetName),
    registry,
    rows: [...generalInfo.rows, ...tableRows.rows],
  };
}

function buildRecordXlsxWorkbookXml(sheetModels) {
  const sheetsXml = sheetModels
    .map(
      (sheetModel, index) =>
        `<sheet name="${escapeXml(sheetModel.name)}" sheetId="${index + 1}" r:id="rId${index + 1}"/>`
    )
    .join("");
...
function buildRecordXlsxWorkbookRelsXml(sheetModels) {
  const relationships = sheetModels
    .map(
      (_sheetModel, index) =>
        `<Relationship Id="rId${index + 1}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet${index + 1}.xml"/>`
    )
    .join("");
  const stylesRelationshipId = sheetModels.length + 1;
...
function buildRecordXlsxContentTypesXml(sheetModels) {
  const worksheetOverrides = sheetModels
    .map(
      (_sheetModel, index) =>
...
function buildRecordXlsxFile() {
  const registry = createMasterXlsxStyleRegistry();
  const styles = createRecordXlsxStylePalette(registry);
  const sheetModels = [
    buildRecordSheetModel("COSTURA", dom.costuraPanel?.querySelector(".costura-table"), registry, styles),
    buildRecordSheetModel("CORTE", dom.cortePanel?.querySelector(".corte-table"), registry, styles),
    buildRecordSheetModel("ACABADOS", dom.acabadoPanel?.querySelector(".acabado-table"), registry, styles),
    buildRecordSheetModel("RESUMEN", dom.resumenPanel?.querySelector(".resumen-table"), registry, styles),
  ].filter(Boolean);
  ...
  const timestamp = new Date().toISOString();
  const files = [
    { name: "[Content_Types].xml", data: buildRecordXlsxContentTypesXml(sheetModels) },
    { name: "_rels/.rels", data: buildMasterXlsxRootRelsXml() },
    { name: "docProps/app.xml", data: buildMasterXlsxAppXml() },
    { name: "docProps/core.xml", data: buildMasterXlsxCoreXml(timestamp) },
    { name: "xl/_rels/workbook.xml.rels", data: buildRecordXlsxWorkbookRelsXml(sheetModels) },
    { name: "xl/styles.xml", data: buildMasterXlsxStylesXml(registry) },
    { name: "xl/workbook.xml", data: buildRecordXlsxWorkbookXml(sheetModels) },
    ...sheetModels.map((sheetModel, index) => ({
      name: `xl/worksheets/sheet${index + 1}.xml`,
      data: buildMasterXlsxWorksheetXml(sheetModel),
    })),
  ];

  return createMasterXlsxZip(files);
}

function downloadRecordExcelFile(fileBytes, fileName) {
  const blob = new Blob([fileBytes], {
    type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
  });
...
function handleExportRecordExcel() {
  const record = getActiveRecord();

  if (!record) {
    showToast("Selecciona una version para exportar.", "info");
    return;
  }

  try {
    const fileBytes = buildRecordXlsxFile();

    if (!fileBytes) {
      showToast("No se pudo preparar el archivo Excel de la version seleccionada.", "error");
      return;
    }

    downloadRecordExcelFile(fileBytes, getRecordExcelFileName(record));
    showToast("Se descargo el archivo Excel de la version seleccionada.", "success");
  } catch (error) {
    showToast(error.message || "No se pudo generar el archivo .xlsx de la version.", "error");
  }
}
...
function updateVersionActionsState() {
  ...
  dom.exportRecordExcelBtn.disabled = !hasActiveRecord;
...
function validateSaveRequirements(formData) {
  ...
  if (firstIncompleteField) {
    return {
      ...
      message:
        "Para guardar la cotizacion debes completar los campos obligatorios de Informacion general de la prenda y al menos una fila completa en Costura.",
    };
  }
  ...
  const formatRules = [
    ...
    },
    {
      ...
      message: "El campo IDEM debe estar vacio, ser 0 o ser un numero entero de 5 digitos.",
    },
    {
  ...
  if (!nonEmptyRows.length && !hasCorteRows && !hasAcabadoRows) {
    return {
      ...
      message:
        "Para guardar la cotizacion debes completar los campos obligatorios de Informacion general y al menos una fila completa en cualquier tabla.",
    };
  }
