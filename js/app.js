...
let codeContextMenuTable = "costura"; // tracks origin table: 'costura' or 'corte'
...
function updateRowProtection(row, protectionValue) {
  const rowData = readRowValues(row);
  const normalizedProtection = roundToDecimals(
    protectionValue > 0 ? protectionValue : APP_CONFIG.defaultProtection,
    2
  );
...
function commitProtectionEdit(input, options = {}) {
  ...
  if (!options.restorePrevious) {
    if (hasDigits) {
      nextProtection = roundToDecimals(AppUtils.safeNumber(normalizedValue), 2);
    }
  ...
  updateRowProtection(row, nextProtection);
}
...
function handleProtectionDoubleClick(event) {
  ...
  if (isCosturaRowEmpty(row) || row.classList.contains("is-invalid")) {
    return;
  }
...
function normalizeGeneralFormFields() {
  ...
  dom.form.idem.value = normalizeShortIntegerValue(dom.form.idem.value);
...
function applyMasterFilters() {
  ...
  if (!activeFilters.length) {
    appState.masterRows = [...appState.masterSourceRows];
    return;
  }

  appState.masterRows = appState.masterSourceRows.filter((row) => {
...
function updateMasterActionState() {
  if (!dom.masterSaveBtn || !dom.masterAddBtn) {
    return;
  }
  ...
  inputs.forEach(input => {
    if (['tiempoEstimadoCorte', 'tiempoEstimadoHabilitado'].includes(input.dataset.field)) {
      ...
      input.addEventListener("contextmenu", (e) => {
        if (!isEditableMode()) return;
        e.preventDefault();
        openCodeContextMenu(row, e.clientX, e.clientY, "corte");
      });
    }
  });
...
function buildEmptyCorteRow() {
  return { operaciones: "", tiempoEstimadoCorte: "", tiempoEstimadoHabilitado: "", proteccion: APP_CONFIG.defaultProtection, area: "", tiempoCorte: "", tiempoHab: "", tiempoCotizacion: "" };
}
...
function createAcabadoRow(rowData = null) {
  ...
  inputs.forEach(input => {
    if (input.dataset.field === "tiempoEstimado") {
...
function buildEmptyAcabadoRow() {
  return { operaciones: "", tiempoEstimado: "", proteccion: APP_CONFIG.defaultProtection, tiempoCotizacion: "" };
}
...
function lookupOperation(code) {
  ...
  const proteccion = APP_CONFIG.defaultProtection;
...
function buildEmptyRow() {
  return {
    ...
    tiempoEstimado: 0,
    tipoMaq: "",
    proteccion: APP_CONFIG.defaultProtection,
    tipoPta: "",
    tiempoMaq: 0,
    tiempoManual: 0,
    tiempoCotizacion: 0,
...
function setInteractionMode(mode) {
  ...
  dom.saveBtn.setAttribute("title", saveTitle);
  dom.saveBtn.setAttribute("aria-label", saveTitle);
...
function switchView(view, options = {}) {
  ...
  if (isMaster) {
    ...
    const targetId = activeTab ? activeTab.dataset.target : 'costuraPanel';
...
async function loadProtoRecords(proto, options = {}) {
  ...
  const response = await AppsScriptAPI.searchByProto(cleanProto);
  const payload = response.data || response;
  ...
  const records = sortVersionRecords(
...
function loadRecord(recordKey, mode = "search_readonly") {
  ...
  dom.form.idem.value = normalizeShortIntegerValue(record.idem || "");
  dom.form.descripcion.value = record.descripcion || "";
  dom.form.estilo.value = record.estilo || "";
  dom.form.tela.value = record.tela || "";
  dom.form.realizadoPor.value = record.realizadoPor || "";
  dom.form.produccionEstimada.value = formatProductionEstimadaValue(record.produccionEstimada || "");
  dom.form.rutasProcesos.value = record.rutasProcesos || "";
...
function validateSaveRequirements(formData) {
  ...
  const formatRules = [
    ...
    },
    {
      ...
      message: "El campo IDEM debe ser un número entero de 1 o 2 dígitos.",
    },
    {
...
async function handleSave() {
  ...
  try {
    ...
    if (saveMode !== "create") {
      ...
      updateFormRecordInfo({ savedAt: response.savedAt });
    }
...
function readFormData() {
  ...
  return {
    ...
    idem: normalizeShortIntegerValue(dom.form.idem.value),
    descripcion: dom.form.descripcion.value.trim(),
    estilo: dom.form.estilo.value.trim(),
    tela: dom.form.tela.value.trim(),
    realizadoPor: dom.form.realizadoPor.value.trim(),
    produccionEstimada: formatProductionEstimadaValue(dom.form.produccionEstimada.value),
    rutasProcesos: dom.form.rutasProcesos.value.trim(),
  };
}
...
function hydrateCatalog(payload) {
  ...
  (payload.basedatos || []).forEach((item) => {
    ...
    appState.catalogByCode.set(normalizedCode, {
      ...
      tiempoEstimado: AppUtils.safeNumber(item.tiempo ?? item.tiempoEstimado),
  ...
  appState.catalogByCode.forEach((item) => {
    const option = document.createElement("option");
